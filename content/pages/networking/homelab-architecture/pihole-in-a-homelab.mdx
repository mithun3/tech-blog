...
If you are ready to deploy a platform-specific instance, continue to [Pi-hole On Raspberry Pi 4B](/pages/hardware/raspberry-pi/pihole-4b) for a dedicated Pi node or [Pi-hole LXC On Proxmox](/pages/proxmox/workloads/pihole-lxc) for the Proxmox-backed path.
...
```mermaid
flowchart LR
  Router["Router DHCP"] --> D1["DNS 1: primary Pi-hole"]
  Router --> D2["DNS 2: secondary Pi-hole"]
```
...
That platform page is here: [Pi-hole On Raspberry Pi 4B](/pages/hardware/raspberry-pi/pihole-4b).

## Related Topics

- [Pi-hole On Raspberry Pi 4B](/pages/hardware/raspberry-pi/pihole-4b) — the dedicated Pi deployment path for a primary or standalone Pi-hole node.
