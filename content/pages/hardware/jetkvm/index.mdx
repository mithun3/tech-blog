...
- For the small browser terminal path, use [JetKVM Admin Terminal On Raspberry Pi 3](/pages/hardware/raspberry-pi/jetkvm-admin-terminal).
- For remote power-on on the Proxmox host, use [Wake-on-LAN For Proxmox](/pages/hardware/jetkvm/wake-on-lan-for-proxmox).
...
- [Wake-on-LAN For Proxmox](/pages/hardware/jetkvm/wake-on-lan-for-proxmox) - arm the host NIC properly so JetKVM can wake the box instead of only watching it stay off.

## Related Topics

- [JetKVM Admin Terminal On Raspberry Pi 3](/pages/hardware/raspberry-pi/jetkvm-admin-terminal) - use a small dedicated Pi as the client-side console terminal.
