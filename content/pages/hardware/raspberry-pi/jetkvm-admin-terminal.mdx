...
- For the base JetKVM device and its recovery role, start with [JetKVM](/pages/hardware/jetkvm).
- For the host-side remote power-on path, use [Wake-on-LAN For Proxmox](/pages/hardware/jetkvm/wake-on-lan-for-proxmox).
...
- [JetKVM](/pages/hardware/jetkvm) - the base device workflow, cabling, first access, and recovery role.
- [Wake-on-LAN For Proxmox](/pages/hardware/jetkvm/wake-on-lan-for-proxmox) - the host-side power-on path when JetKVM should wake the machine instead of only watching it stay off.
- [Raspberry Pi](/pages/hardware/raspberry-pi) - the rest of the Pi-specific infrastructure guides.
