...
If remote recovery is part of the design, do not treat that row as a universal rule. Read [Wake-on-LAN For Proxmox](/pages/hardware/jetkvm/wake-on-lan-for-proxmox) and enable it on purpose instead of by habit.
...
- If you need baremetal hardware access alongside Proxmox, continue into [Dual Boot: Proxmox & Rocky Linux](/pages/proxmox/setup/rocky-linux-dual-boot).
...
- [Dual Boot: Proxmox & Rocky Linux](/pages/proxmox/setup/rocky-linux-dual-boot) — carving out baremetal access while preserving Proxmox ZFS redundancy.
