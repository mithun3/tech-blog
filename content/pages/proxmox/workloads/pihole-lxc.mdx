...
The networking-side design lives in [Pi-hole In A Homelab](/pages/networking/homelab-architecture/pihole-in-a-homelab). The dedicated Pi build lives in [Pi-hole On Raspberry Pi 4B](/pages/hardware/raspberry-pi/pihole-4b). This page only covers the Proxmox workload layer.
...
- [Pi-hole On Raspberry Pi 4B](/pages/hardware/raspberry-pi/pihole-4b) — the dedicated Pi deployment path when the DNS node should live on its own hardware.
