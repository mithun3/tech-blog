...
# Cloudflare Tunnel On Proxmox (Legacy)

> [!WARNING]  
> **Deprecated for Administrative Access.** 
> While Cloudflare Tunnels hide your public IP and provide a secure reverse proxy, they still fundamentally expose an endpoint to the public internet. This architecture has shifted to a strict **Zero Trust Network Access (ZTNA) overlay**. For all remote access to management interfaces, you must use the [Tailscale ZTNA & Subnet Routing](/pages/networking/homelab-architecture/tailscale-ztna) model. Cloudflare Tunnels should only be used when explicitly required to share a service with the general public.

This is the legacy path for the lab that wants secure public sharing without opening the firewall and without turning the Proxmox host into an internet-facing snowflake.
...
- [OpenClaw On Proxmox](/pages/proxmox/workloads/openclaw) - the assistant workload that can expose its control UI through this same tunnel layer without opening inbound ports.
