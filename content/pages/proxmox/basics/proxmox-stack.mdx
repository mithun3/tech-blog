...
That is exactly where the k3s cluster in [Kubernetes On Proxmox With k3s](/pages/proxmox/workloads/kubernetes) fits: Proxmox stays the machine layer, and Kubernetes lives inside guests that can be snapshotted, rebuilt, and upgraded without turning the host into the experiment.
