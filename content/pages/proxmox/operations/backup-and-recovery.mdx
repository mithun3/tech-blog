...
PBS is the next step once local `vzdump` files start competing with the host they are supposed to protect.

In this lab, PBS moved daily container backups from `rpool/var-lib-vz` to a dedicated `pbspool` SSD through CT 301. That change freed roughly 1.1 TB from the root pool and made daily backups deduplicated, visible, and easier to restore from.

Use [Proxmox Backup Server](/pages/proxmox/operations/proxmox-backup-server) when the question is no longer "can I make a backup?" but "can I keep backups without filling the host, test restores monthly, and survive a real upgrade window?"
...
The `--mailnotification` flag is legacy, but these are the validated lines from the original host. If you are building fresh today, pair this page with [Email Notifications](/pages/proxmox/operations/email-notifications) and decide whether you want to keep the legacy mail flag or move fully to the notification system.[^1]

If backups have already moved onto the NAS VM, swap the storage target to `truenas-vzdump` using [TrueNAS Shares And Proxmox Integration](/pages/proxmox/workloads/truenas-scale/shares-and-proxmox-integration).
...
- [Proxmox Backup Server](/pages/proxmox/operations/proxmox-backup-server) — the dedicated PBS path for daily deduplicated backups and restore practice.
