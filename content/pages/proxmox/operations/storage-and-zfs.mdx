...
If reused disks have not been validated yet, stop here and do that first in [Used Drive Validation Before Deployment](/pages/proxmox/setup/used-drive-validation). Pool design is not the place to discover that cheap capacity was actually deferred failure.
...
If the lab is moving in that direction, treat this section as the host-side baseline and use [TrueNAS SCALE On Proxmox](/pages/proxmox/workloads/truenas-scale) for the actual migration path, NFS and SMB layout, and hot-spare workflow.
...
- [Used Drive Validation Before Deployment](/pages/proxmox/setup/used-drive-validation) — the destructive disk preflight that should happen before reused drives join a real pool.